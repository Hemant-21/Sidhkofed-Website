...
--
-- WHEN IT RUNS:
--   * run on demand via `npm run db:test:create`;
--   * `npm run db:test:setup` runs this file, migrations, then seed.
--
-- The development database (sidhkofed_cms) is NEVER touched here. Run as a PostgreSQL admin
-- login so it can create the role, the database, and (re)assign public-schema ownership the
-- test role needs to run migrations (PG15+ locks the public schema down by default).
