...
import { parsePublicationState, parseSearch } from '@/shared/list-query';
...
const list = wrap(async (req) => {
  ...
  // Validate the enum BEFORE it reaches the service/Prisma — invalid → 422 (no manual cast).
...
const updateImage = wrap(async (req) => {
...
const removeImage = wrap(async (req) => {
