...
async function create(data: MediaCreateInput) {
...
async function findByChecksum(checksum: string) {
...
async function list(filters: MediaListFilters, skip: number, take: number, direction: 'asc' | 'desc') {
...
async function setArchived(id: string, archivedAt: Date | null) {
  return prisma.mediaAsset.update({ where: { id }, data: { archivedAt } });
}

async function setReplacedBy(oldId: string, newId: string) {
